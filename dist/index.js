 ...
 */
function extractFileAIInfo(note, commitSha) {
    const fileInfos = [];
    try {
 ...
 */
async function addInlineComments(octokit, owner, repo, prNumber, notes) {
    try {
        // Extract all file AI info from notes
        const fileAIMap = new Map();
        ...
        const botReview = existingReviews.find(review => review.body?.includes('🤖 AI-Modified Files'));
        if (botReview) {
